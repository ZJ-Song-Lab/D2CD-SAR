...
"""
Inference script for SAR Ship Detection using DINOv3 ViT-7B/16
...
class SARDetectorInference:
    """SAR Ship Detector for inference with ViT-7B/16"""
    ...
    def _build_model(self):
        """Build detection model with ViT-7B/16"""
        ...
        # Load ViT-7B/16 backbone
        print("Loading DINOv3 ViT-7B/16 backbone...")
        backbone = dinov3_vit7b16(pretrained=False, weights=None, check_hash=False)

        # Configure for ViT-7B (3x3 windows)
...
def visualize_detection(
    ...
    ax.set_title("SAR Ship Detection Results (ViT-7B/16)")
    ax.axis("off")
...
def visualize_heatmap(
    ...
    axes[1].set_title("Attention Heatmap (ViT-7B/16)")
    axes[1].axis("off")
...
def detect(
    image_path: str,
    checkpoint_path: str,
    output_dir: str = "./outputs/inference_vit7b",
    device: str = "cuda",
    conf_threshold: float = 0.5,
    img_size: int = 896,
):
    ...
    # Initialize detector
    print("Initializing ViT-7B/16 detector...")
...
def main():
    parser = argparse.ArgumentParser("SAR Ship Detection Inference with ViT-7B/16")
    ...
    parser.add_argument("--output-dir", default="./outputs/inference_vit7b", type=str, help="Output directory")
